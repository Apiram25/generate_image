...
for image_idx in range(50):
    ...
    height = 700
    width = 500
    vp_height = np.random.randint(-800,-200)
    vp_width = np.random.randint(125,375)
    vanishing_point = (vp_width, vp_height)
    offset = np.random.randint(-50,50)

    # Plants generation parameters
    nb_of_plants_rows = np.random.randint(4,8)
    inter_row_plants_distance = int(width / (nb_of_plants_rows-1))
    ...
    for i in range(nb_of_plants_rows):
        ...
        cv.line(img, (i * inter_row_plants_distance + offset, height), vanishing_point, 255, 1)
